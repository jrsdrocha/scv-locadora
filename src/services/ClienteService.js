...
class ClienteService {
    ...
    static async create(req) {
        ...
        const t = await sequelize.transction();
        ...
        try {
            ...
            await t.rollback();
            throw "Problema na inserção de um dos telefone do cliente!"
        }
    }
    ...
    static async update(req) {
        const { id } = req.params;
        const { nome, cpf, rua, numero, debito, nascimento, bairro, telefones } = req.body;
        if (bairro == null) throw "O Bairro do Clienete deve ser preenchido!"
        const obj = await Cliente.findByPk(id, { include: { all: true, nested: true } });
        ...
        try {
            await Promise.all((await obj.telefones).map(telefone => telefone.destroy({ transaction: t }))); //Apaga todos telefones deste Cliente.
            await Promise.all(telefones.map(telefone => obj.createTelefone(telefone, { transaction: t }))); //insere todos os Telefones do novo requeste, exceto os ja existentes.
            await t.commit();
            return await Cliente.findByPk(obj.id, { include: { all: true, nested: true } });
        } catch (error) {
            await t.rollback();
    ...
    static async delete(req) {
        ...
        if (obj == null) throw "Cliente nao encontrado!";
        try {
            await obj.destroy();
            return obj;
        } catch (error) {
            throw "Não é possível remover um cliente associado a um empréstimo ou reservas!"
        }
    }
